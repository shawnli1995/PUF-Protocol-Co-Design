...
def initialize_and_tranform_PUF(n: int, k: int, N: int, seed_sim: int, noisiness: float, interface: bool,
    ...
    if puf == "lppuf":
        ...
        puf = lppuf.LPPUFv1(n=n, m=k, seed=seed_sim + 0, noisiness_1=noisiness, noisiness_2=noisiness)
...
def run(n: int, k: int, N: int, seed_sim: int, noisiness: float, BATCH_SIZE: int, interface: bool,
    ...
    challenges2 = copy.deepcopy(challenges)
    challenges = np.cumprod(np.fliplr(challenges), axis=1, dtype=np.int8)
    challenges = np.concatenate((challenges2, challenges), axis=1)
    ...
    model.add(
        layers.Dense(64, activation='tanh', input_dim=(n + ghost_bit_len)*2, kernel_initializer='random_normal'))
    model.add(layers.Dense(32, activation='tanh'))
    model.add(layers.Dense(32, activation='tanh'))
    ...
    if puf == "ffpuf":
        ...
        if interface == 0:
            with open('lppuf_without_interface.csv', 'a') as f:
                # using csv.writer method from CSV package
                write = csv.writer(f)
                # if seed_sim == 0:
                #     write.writerow(fields)
                write.writerows(rows)
        if interface == 1:
            with open('lppuf_interface_plus.csv', 'a') as f:
                # using csv.writer method from CSV package
                write = csv.writer(f)
                if seed_sim == 0:
                    write.writerow(fields)
...
def main(argv=None):
    ...
    interface = False
    for i in range(3):
        run(32, 4, 1000000, i, 0.00, 1000, interface, 0, 0, puf)
    interface = True
    for i in range(3):
        run(32, 4, 40000000, i, 0.00, 100000, interface, 20, 0, puf)
    # for i in range(10):
